...
export function GoodBadCompare({ data }: Props) {
  const [mode, setMode] = useState<'stacked' | 'dual'>('stacked')

  return (
    <div className="space-y-4">
      <div className="flex items-center gap-2">
        {(['stacked', 'dual'] as const).map((option) => (
          <button
            key={option}
        ...
        <ResponsiveContainer width="100%" height="100%">
          <BarChart data={data}>
            ...
            <Bar
              dataKey="good"
              fill="#22c55e"
              stackId={mode === 'stacked' ? 'compare' : undefined}
              radius={[4, 4, 0, 0]}
            />
            <Bar
              dataKey="bad"
