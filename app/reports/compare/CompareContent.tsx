...
export default function CompareContent() {
  ...
  useEffect(() => {
    ...
    async function loadReports() {
      ...
      try {
        ...
        if (!active) return
        setReportsError(err instanceof Error ? err.message : 'Erreur inconnue.')
        setReport1(null)
        setReport2(null)
      } finally {
  ...
  useEffect(() => {
    ...
    async function loadAnalysis() {
      ...
      try {
        // ---- FIX FINAL : PAS DE NULL DANS URLSearchParams ----
        const params: Record<string, string> = {}
        if (id1 !== null) params.id1 = id1
        if (id2 !== null) params.id2 = id2
  ...
  // ----------------------------------------
  const diff = useMemo(
    () => diffText(report1?.report, report2?.report),
    [report1?.report, report2?.report]
  )

  // ----------------------------------------
  //     PAS D’ID → MESSAGE D’ERREUR
  // ----------------------------------------
  if (!id1 || !id2) {
    return (
      <div className="min-h-screen bg-gray-950 text-white">
        <div className="max-w-4xl mx-auto px-4 py-10">
          ...
          <p className="text-gray-400 mb-4">
            Merci de sélectionner deux rapports depuis l&apos;historique.
          </p>

          <Link
            href="/reports/history"
            className="inline-flex items-center gap-2 rounded-xl border border-white/10 px-4 py-2 text-sm text-gray-200 hover:border-white/30"
          >
    ...
    <main className="min-h-screen bg-gray-950 text-white">
      ...
      <div className="max-w-6xl mx-auto px-4 py-8 space-y-10">
        ...
        <section className="rounded-lg border border-gray-800 bg-gray-900/70 p-6">
          ...
          {reportsLoading && <p className="text-sm text-gray-400">Récupération...</p>}
          {reportsError && <p className="text-sm text-red-400">{reportsError}</p>}
          {!reportsLoading && !reportsError && (
            <p className="text-sm text-gray-400">Comparaison des rapports #{id1} et #{id2}</p>
          )}
        </section>
        ...
        <section className="rounded-lg border border-gray-800 bg-gray-900/70 p-6 space-y-4">
          <div className="flex items-center justify-between">
            ...
            {analysisLoading && (
              <span className="text-xs uppercase tracking-wide text-gray-400">Analyse en cours...</span>
            )}
          </div>
          ...
          {analysis && (
            <div className="space-y-4">
              ...
              <div>
                <h3 className="font-semibold text-blue-400">Score</h3>
          ...
          {!analysisLoading && !analysis && !analysisError && (
            <p className="text-sm text-gray-400">Analyse indisponible.</p>
          )}
        </section>
        ...
        <section className="rounded-lg border border-gray-800 bg-gray-900/70 p-6">
          ...
          {!diff.length && <p className="text-sm text-gray-400">Calcul des différences...</p>}
          ...
          {diff.length > 0 && (
            <pre className="whitespace-pre-wrap text-sm space-y-1">
              {diff.map((line, idx) => {
                ...
                if (line.type === 'add') {
                  return <div key={idx} className="text-green-400">+ {line.text}</div>
                }
