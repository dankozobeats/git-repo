...
export async function POST(request: NextRequest) {
  try {
    ...
    const { data: logs } = await supabase.from('logs').select('*').eq('user_id', user.id).gte('completed_date', startDateStr).order('completed_date', { ascending: true })
    const { data: events } = await supabase.from('habit_events').select('*').eq('user_id', user.id).gte('event_date', startDateStr).order('event_date', { ascending: true })
